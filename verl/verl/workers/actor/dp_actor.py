...
class DataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def update_unlearn_policy(self, data: DataProto):
        """A temporary unlearn policy update to avoid dominant mode, used for DCRL"""
        ...
        metrics = {
            "unlearn/epsilon_L": self.config.dcrl.epsilon_low,
            "unlearn/epsilon_R": self.config.dcrl.epsilon_high,
            "unlearn/ratio": self.config.dcrl.unlearn_ratio
        }

        self.gradient_accumulation = (
                self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
        ) if not self.config.use_dynamic_bsz else 1

        for epoch in range(self.config.get("unlearn_epochs", 1)):
            epoch_loss = 0.0
            for batch_idx, mini_batch_data in enumerate(dataloader):
                ...
                for micro_batch in micro_batches:
                    ...
                    p_clip = torch.clamp(probs, self.config.dcrl.epsilon_low, 1 - self.config.dcrl.epsilon_high)
                    unlearn_loss = -torch.log(1 - p_clip)
                    unlearn_loss = agg_loss(
                        loss_mat=unlearn_loss,
                        loss_mask=response_mask,
                        loss_agg_mode=self.config.loss_agg_mode
                    )

                    if self.config.use_dynamic_bsz:
                        loss = unlearn_loss * (len(micro_batch) / self.config.ppo_mini_batch_size)
                    else:
                        loss = unlearn_loss / self.gradient_accumulation
                    loss.backward()
                    batch_loss += unlearn_loss.detach().item()

                old_lr = self.actor_optimizer.param_groups[0]["lr"]
                for param_group in self.actor_optimizer.param_groups:
                    param_group["lr"] = self.config.dcrl.unlearn_ratio * old_lr
                metrics[f"unlearn/unlearn_lr"] = self.actor_optimizer.param_groups[0]["lr"]
                grad_norm = self._optimizer_step()
                for param_group in self.actor_optimizer.param_groups:
                    param_group["lr"] = old_lr

                batch_avg_loss = batch_loss / len(micro_batches)
                epoch_loss += batch_avg_loss
                metrics[f"unlearn/batch_{batch_idx}/loss"] = batch_avg_loss
                metrics[f"unlearn/batch_{batch_idx}/grad_norm"] = grad_norm.detach().item()

            metrics[f"unlearn/epoch_{epoch}/loss"] = epoch_loss / len(dataloader)
